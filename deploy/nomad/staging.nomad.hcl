variable "image" {
  ...
  validation {
    condition     = strlen(var.image) == 106 && substr(var.image, 0, 42) == "ghcr.io/sachahjkl/cnam_a2_si_musee@sha256:"
    error_message = "The image must use the Musee Lookout GHCR repository and an exact SHA-256 digest."
  }
}
